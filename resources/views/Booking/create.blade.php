            ...
            <form action="{{ route('booking.store') }}" method="POST" class="booking-form" id="bookingForm">
                ...
                <div class="form-section">
                    ...
                    <div class="room-options" id="roomOptions">
                        @forelse($availableRoomTypes as $tipe)
                            <label class="room-option">
                                <input type="radio" name="tipe_kamar_id" value="{{ $tipe->id }}" required
                                       data-price="{{ $tipe->harga }}"
                                       {{ ($selectedTipeKamarId && $selectedTipeKamarId == $tipe->id) || $loop->first ? 'checked' : '' }}>
                                <div class="room-header">
                                    <div class="room-info">
                                        <div class="room-name">{{ $tipe->nama_tipe }}</div>
                                        <div class="room-facilities">{{ $tipe->deskripsi ?? 'Fasilitas standar' }}</div>
                                        <div class="room-capacity">
                                            <i class="fas fa-users"></i>
                                            <span>Maksimal {{ $tipe->kapasitas }} tamu</span>
                                        </div>
                                        @if($tipe->fasilitas_kamar)
                                            <div class="room-features">
                                                @php
                                                    $features = is_string($tipe->fasilitas_kamar)
                                                        ? json_decode($tipe->fasilitas_kamar, true) ?? explode(',', $tipe->fasilitas_kamar)
                        ...
                        @empty
                            <div style="text-align: center; padding: 40px; background: #fee; border: 1px solid #fcc; border-radius: 8px;">
                                <i class="fas fa-exclamation-triangle" style="font-size: 48px; color: #c33; margin-bottom: 16px;"></i>
                                <p style="color: #c33; font-weight: 600; margin-bottom: 16px;">
                                    Maaf, tidak ada kamar tersedia untuk tanggal yang Anda pilih
                                </p>
                                <a href="{{ route('property.show', $property->id) }}"
                                   style="display: inline-block; padding: 12px 24px; background: var(--accent); color: var(--primary); text-decoration: none; border-radius: 8px; font-weight: 600;">
                ...
                <!-- GUEST INFORMATION (Tampil jika belum login) -->
                @guest
                <div class="form-section">
                    <h3 class="section-title">
                    ...
                    <div style="background: rgba(212, 175, 55, 0.05); padding: 16px; border-radius: 8px; margin-top: 16px;">
                        <label style="display: flex; align-items: start; gap: 12px; cursor: pointer;">
                            <input type="checkbox" name="create_account" value="1" id="createAccountCheckbox"
                                   style="display: block; margin-top: 4px; width: 18px; height: 18px; cursor: pointer;">
                            <div>
